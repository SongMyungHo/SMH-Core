...
SMHVER=($($SONGMYUNGHOCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $SONGMYUNGHOD $SONGMYUNGHOCLI $SONGMYUNGHOTX $SONGMYUNGHOQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${SMHVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${SMHVER[1]}//g" ${MANDIR}/${cmdname}.1
done
